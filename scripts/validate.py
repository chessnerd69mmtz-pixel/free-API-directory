...
errors=[]
statuses={'active','needs re-verification','broken-link','discontinued'}
...
for i,p in enumerate(data,1):
    ...
    for field in ('signup_url','pricing_url'):
        u=p.get(field,''); q=urlparse(u)
        if q.scheme!='https' or not q.netloc: errors.append(f'{i}: invalid {field}: {u}')
    try: date.fromisoformat(p.get('last_verified',''))
    except ValueError: errors.append(f'{i}: invalid last_verified')
    if not p.get('verified_by'): errors.append(f'{i}: missing verified_by')
    if p.get('signup_url')==p.get('pricing_url'): errors.append(f'{i}: source URLs must differ')
