...
function renderVault(){const q=new URLSearchParams(location.search).get("provider");document.querySelector("#app").innerHTML='<div class="wrap">'+nav()+'<section class="hero"><h1>🔐 My API Keys</h1><p>Encrypted with AES-GCM and stored only on this browser. Nothing is uploaded to the directory.</p></section><div class="card tool"><select id="provider" class="select"><option value="">Choose provider…</option>'+options(q)+'</select><input id="label" class="input" placeholder="Label (optional)"><input id="key" class="input" type="text" name="api-key-value" placeholder="Paste API key or token" autocomplete="off" autocapitalize="off" spellcheck="false"><button id="save" class="btn">Save key</button><button id="lock" class="btn secondary">Lock</button><button id="export" class="btn secondary">Export encrypted vault</button><button id="import" class="btn secondary">Import vault</button><input id="file" type="file" accept=".json" hidden></div><div id="msg"></div><div class="card"><h2>Saved credentials</h2><div id="list"></div></div></div>';$("#save").onclick=save;$("#lock").onclick=()=>{vaultKey=null;records=[];renderLogin()};$("#export").onclick=exportVault;$("#import").onclick=()=>$("#file").click();$("#file").onchange=importVault;renderList()}
...
async function importVault(){try{const file=$("#file").files[0];if(!file)return;const d=JSON.parse(await file.text());if(d.version!==1||!d.salt||!Array.isArray(d.records))throw 0;if(!confirm("This replaces the encrypted vault in this browser. Continue?"))return;localStorage.setItem("apiVaultSalt",d.salt);if(d.verifier)localStorage.setItem("apiVaultVerifier",d.verifier);for(const r of await all())await del(r.id);for(const r of d.records)await put(r);location.reload()}catch(_){msg("That vault backup is invalid or could not be imported.")}}
