...
function renderVault(){const q=new URLSearchParams(location.search).get("provider");document.querySelector("#app").innerHTML='<div class="wrap">'+nav()+'<section class="hero"><h1>🔐 My API Keys</h1><p>Encrypted with AES-GCM and stored only on this browser. Nothing is uploaded to the directory.</p></section><div class="card tool"><select id="provider" class="select"><option value="">Choose provider…</option>'+options(q)+'</select><input id="label" class="input" placeholder="Label (optional)"><input id="key" class="input" type="password" placeholder="Paste API key or token"><button id="save" class="btn">Save key</button><button id="lock" class="btn secondary">Lock</button><button id="export" class="btn secondary">Export encrypted vault</button><button id="import" class="btn secondary">Import vault</button><input id="file" type="file" accept=".json" hidden></div><div id="msg"></div><div class="card"><h2>Saved credentials</h2><div id="list"></div></div></div>';$("#save").onclick=save;$("#lock").onclick=()=>{vaultKey=null;records=[];renderLogin()};$("#export").onclick=exportVault;$("#import").onclick=()=>$("#file").click();$("#file").onchange=importVault;renderList()}
